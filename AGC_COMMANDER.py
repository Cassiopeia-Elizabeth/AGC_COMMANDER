...
from PyQt5.QtWidgets import QWidget, QApplication, QPushButton, QMenu, QMenuBar, QMainWindow, QLabel, \
    QStatusBar, QFrame, QComboBox, QCheckBox, QLCDNumber
...
class AGCUI(QMainWindow):
    def __init__(self):
        ...
        # import my ui
        uic.loadUi("AGC_Mainmk2.ui", self)
        # define widgets
        ...
        self.port_A = self.findChild(QLabel, "port_A")
        self.port_B = self.findChild(QLabel, "port_B")
        self.port_C = self.findChild(QLabel, "port_C")
    ...
    def pos_status(self):
        ...
        if data_received[0:1] == expected_response:
            ...
            scale = 16
            num_of_bits = 8
            binA = bin(int(portA, scale))[2:].zfill(num_of_bits)
            binB = bin(int(portB, scale))[2:].zfill(num_of_bits)
            binC = bin(int(portC, scale))[2:].zfill(num_of_bits)
            self.port_A.setText(str(binA))
            self.port_B.setText(str(binB))
            self.port_C.setText(str(binC))
    ...
    def auto_disable(self):
        ...
        data = pd.read_csv("antenna_positions.csv")
